...
export const getYearMonthDay = function(sy, sm, sd) {
	...
	ld = hasDayOfRest(ld, ldNum, lm,sy, (sm + 1), sd);
	return ld;
};

//计算节假日
function hasDayOfRest(ld, ldNum, lm,year, month, day) {
	switch (lm) { //判断农历
		case '五':
			if (ldNum == 5) {
				return '端午节'
		...
		case '八':
			...
			break;
		case '腊':
			if (ldNum == months[months.length - 1]) {
				return '除夕'
			}
			break;
		case '正':
	...
	if (month == 1 && day == 1) {
		...
		return '妇女节'
	}else if (month == 4 && day == getQinMingJieDate(year)) {
		return '清明节'
